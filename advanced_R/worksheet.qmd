...
$$\text{earnings}_i = \beta_0 + \beta_1 \, \text{height}_i + \beta_2  \, \text{female}_i + \epsilon_i$$
...
A data scientist might work for a company that wants to use this data to offer personal styling packages priced relative to clients’ income. That is, we are not interested in _why_ salary differs, we just want a model that makes good predictions.
